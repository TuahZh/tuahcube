#!/usr/bin/env python
#filename=channel_maps_v1
#---------------------------------------by Gao John
#this file is to generate the integretion of the cube in CO observation from velocity argv[1] to argv[2] by 1km/s
...
#============================================
#prepare figure & axes
fig = figure(1, figsize=(9,12), dpi=70)
...
for i, ax in enumerate(g):
   ...
   # channel = co_data[channel_number]
    ...
    k = 0
    while (k<nn):
        if (vn[k]<=vcut1+1 and vn[k+1]>=vcut1+1):
            kcut2 = k
            break
        k+=1
...
#        #ns = 0.
    ...
    #draw contour
    cont = ax[radio_header].contour(radio_data, linewidth=0.5, levels=arange(0.,1.2,0.1), colors="w", alpha=0.2)
    v = vcut1
    t = ax.add_inner_title(r"$%4.1f$ km s$^{-1}$" % (v), loc=4, frameon=False)
    if use_path_effect:
...
savefig("co12_channel_maps_%d"%vcut_for_name+"_v1.eps", bbox_inches="tight")
#
